...
`staged_regt` 保持两阶段顺序：先用一个共享 worker 池并行减多头和回补空头，刷新持仓与购买力并重新优化后，再并行建立目标多空仓位。减仓订单仍分别记录为 `release_sell_long` 和 `release_buy_to_cover`，但同属 `reduce_exposure` 批次。marketable-limit 每次使用不超过 `5s` 的实时 bid/ask 定价，每个标的跨 release 重建轮次合计最多尝试 `4` 个不重复报价档位（`0/25/75/150 bps`，每档等待 `6s`）。entry 完成后会重新同步仓位，只对最终未成交且仍有可执行权重缺口的标的按误差从大到小进行一次残差修复；修复默认仅提交一个受 `150 bps` 下限/上限保护的限价尝试。审计会分别记录正常改价撤单、修复成功和最终漏单。
