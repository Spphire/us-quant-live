...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        positions_after = client.list_positions()
    ...
    *,
    ...
    for symbol in symbols:
        ...
        if release_long:
            ...
            raw_qty = min(float(raw_qty), max(0.0, abs(float(signed_qty))))

        qty = _quantize_qty(raw_qty, whole_shares_only=should_force_whole_share, decimals=qty_decimals)
        if qty <= 0:
            ...
            records.append(
                {
                    **base_record,
                    ...
                    "remaining_qty": float(item.qty),
                    "error": str(exc),
                    "abort_remaining_orders": bool(_is_insufficient_buying_power_error(exc)),
                }
            )
            if _is_insufficient_buying_power_error(exc):
                break
...
def _submit_staged_regt_orders(
    *,
    ...
    _, entry_instructions = _split_release_entry_instructions(entry_instructions)
